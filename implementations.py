import numpy as np
from utils.algo import compute_gradient, compute_loss, compute_loss_lr
...
def ridge_regression(y, tx, lambda_):
    ...
    :return Tuple<>: optimal_weights and mse
    """
    optimal_weight = np.linalg.solve(np.dot(tx.T, tx) + lambda_ * 2 * tx.shape[0] * np.identity(tx.shape[1]),
                                     np.dot(tx.T, y))
...
def logistic_regression(y, tx, initial_w, max_iters, gamma):
    ...
    for n_iter in range(max_iters):
        for minibatch_y, minibatch_tx in batch_iter(y, tx, 1):
            # compute gradient and loss
            loss, gradient = compute_loss_lr(minibatch_y, minibatch_tx, w)
            # update w by gradient
            w = w - gamma * gradient
            # store w and loss
            ws.append(w)
